...
const HeroSection = () => {
    ...
    const ButtonGroup = ({ next, previous, ...rest }) => {
        ...
        return (
            <>
                <button className={currentSlide === 0 ? "hidden" : "absolute left-9 flex justify-center items-center w-8 h-8 rounded-full p-1 text-white bg-black/70 hover:bg-gray-800"} onClick={() => previous()} ><IoIosArrowBack size={35} /></button>
                <button className="absolute right-9 flex justify-center items-center w-8 h-8 rounded-full p-1 text-white bg-black/70 hover:bg-gray-800" onClick={() => next()} ><IoIosArrowForward size={35} /></button>
            </>
        );
    };
    ...
    return (
        <>
            ...
            <div className="fixed right-7 bottom-8 flex flex-col gap-y-3 items-center">
                <div className="bg-green-500 flex justify-center items-center p-2 cursor-pointer text-white rounded-full">
                    <FaWhatsapp size={35} />
                </div>
                <button className="flex justify-center items-center w-8 h-8 rounded-full p-1 text-white bg-black/70 hover:bg-gray-800" ><IoIosArrowUp size={35} /></button>
            </div>
        </>
    );
};
